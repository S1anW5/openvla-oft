"""
finetune_svgd_ensemble.py

Stage 1: Train K=2 LoRA adapters on a shared frozen base model with SVGD
...
the ensemble.
  ...
  L_repulsion = exp(-d / max(h_ema, 1e-6))
  d = mean_over_batch ||flatten(pred_0) - flatten(pred_1)||_2^2
  h_ema = 0.9 * h_{t-1} + 0.1 * d.detach()   (every step, including warmup)

  λ(t): 0 for t < warmup_steps, linear ramp, then constant λ_max
  t = effective gradient step (= micro_step // grad_accum_steps)
"""
...
def train(cfg: SVGDEnsembleConfig) -> None:
    ...
    run_id = (
        f"svgd-K{cfg.num_particles}-r{cfg.lora_rank}"
