#!/bin/bash
# Official OFT recipe: LIBERO-Goal, single GPU (GPU 2)
# Effective batch = 1 x grad_accum 8 = 8
# Evaluates best at 50K steps (paper finding)
# 推荐运行方式: bash train_official_goal.sh 2>&1 | tee runs/logs/official_goal.log
...
WANDB_MODE=disabled CUDA_VISIBLE_DEVICES=2 torchrun --standalone --nnodes 1 --nproc-per-node 1 vla-scripts/finetune.py \
  ...
  --run_root_dir /hdd/slwu/test_5_2/openvla-oft/runs/official_repro \
