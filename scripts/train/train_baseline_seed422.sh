#!/bin/bash
# Baseline seed=422: second seed for statistical significance
# GPU 0, 50K steps (~43 hours)
# 推荐运行方式: bash train_baseline_seed422.sh 2>&1 | tee runs/logs/baseline_seed422.log
...
WANDB_MODE=disabled CUDA_VISIBLE_DEVICES=0 torchrun --standalone --nnodes 1 --nproc-per-node 1 vla-scripts/finetune.py \
  ...
  --run_root_dir /hdd/slwu/test_5_2/openvla-oft/runs/comparison \
